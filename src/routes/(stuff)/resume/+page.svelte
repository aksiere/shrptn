	...
	<div class='flex align-flex-end'>
		...
		<div class='1/1'>
			{#if show}
				...
				<p in:animate={{ y: 90, duration: TRANSITION_DURATION }} class='text-justify'>Свои проекты делаю на SvelteKit, вместо классических препроцессоров использую UnoCSS + Tailwind (плюс делаю свой фреймворк для более быстрой адаптивной верстки), в качестве сборщика - Vite.</p>
		...
		<div class='1/1 text-center mb-0 mt-3'>
			{#if show}
				...
				<p class='m-0' in:animate={{ y: 150 + (skills.length * 10) + (educations.length * 10) + (jobs.length * 10), duration: TRANSITION_DURATION }} style='color: #555;'><a href='/resume.pdf'>pdf-версия</a></p>
			{/if}
		</div>
	</div>
</div>
